...
public class Server {
    ...
    class ConnectionHandler extends Thread {
        ...
        public void getSavedGames(){
            Vector<Game> allGames = DBManager.getPlayerSavedGames(currentPlayerUsername);
            List<JSONObject> gamesList = new ArrayList<>();
            for(Game g: allGames){
                Sjson = new JSONObject();
                Sjson.put("id", g.getID());
                Sjson.put("player1", g.getP1());
                Sjson.put("player2", g.getP2());
                Sjson.put("TS", g.getTS());
                gamesList.add(Sjson);
            }
            Sjson = new JSONObject();
            Sjson.put("code", "GETSAVEDGAMES");
            Sjson.put("allgames", gamesList);
            clientPrintStream.println(Sjson);
            System.out.println(Sjson);
        }
    }
